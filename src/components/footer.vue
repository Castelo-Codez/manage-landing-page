...
onMounted(() => {
    ...
    form.addEventListener("submit", (e) => {
        let $result = $checkError(form.email.value);
        if ($checkObjectLength($result)) {
            e.preventDefault();
            for (let prop in $result) {
                if ($result.hasOwnProperty(prop)) {
                    $runError($result[prop]);
                }
            }
        } else {
            $removeError();
    ...
    function $validation($email) {
        let $Obj = {};
        if ($email !== "") {
            validator.isEmail($email) ? "" : ($Obj.error = "invalid Email");
        } else {
            $Obj.error = "Email cannot Be Empty";
        }
        return $Obj;
    }
    function $checkObjectLength($oj) {
        let $num = 0;
        for (let prop in $oj) {
            $num++;
        }
                ...
                &:nth-child(2) {
                    ...
                    > ul {
                        li {
                            margin-bottom: 20px;
                            a {
                                color: var(--Neutral-VaryLightGray);
                                transition: 0.2s;
